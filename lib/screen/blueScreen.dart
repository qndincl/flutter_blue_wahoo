...
//TODO: 클릭 2번시 disconnect 구현 - 됨 => 다른거 눌렀을때 꺼지는지, 아니면 그거랑 연결이 되는지 암튼 에러남
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Obx(
        () => ListView.builder(
            itemCount: bluetoothTile.length,
            itemBuilder: (context, index) {
              return GestureDetector(
                onTap: () {
                  ...
                  // );
                  // print("isConnecting2 : $_isConnecting");
                  // if (!_isConnecting) {
                  //   _isConnecting = true;
                  //   print("_isConnecting $_isConnecting");
                  isloading = !isloading;
                ...
                child: ListTile(
                  title: Text(
                    'name : ${bluetoothTile[index].device.name}',
                  ),
                  subtitle: Text(bluetoothTile[index].device.id.toString()),
                  trailing: Text(bluetoothTile[index].rssi.toString()),
      ...
      floatingActionButton: FloatingActionButton(
        ...
        child: IconButton(
          icon: _isScanning ? Icon(Icons.stop) : Icon(Icons.play_arrow),
          onPressed: () {
            if (_isScanning) {
              print('_isScanning1 $_isScanning');
              setState(() {}); //icon 모양 바꾸기 위한 setState
              stop();
            } else {
              print('_isScanning22 $_isScanning');
              setState(() {});
              scan();
            }
          },
